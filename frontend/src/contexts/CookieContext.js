...
export const setUserCookies = function (
    username,
    token,
    refreshToken
) {
    cookies.set(
        COOKIES_USERNAME, username, { maxAge: ONE_MONTH_AGE }
    );
    cookies.set(
        COOKIES_REFRESH_TOKEN, refreshToken, { maxAge: ONE_MONTH_AGE }
    );
    cookies.set(
        COOKIES_TOKEN, token, { maxAge: FIVE_MIN_AGE }
    );
}
